...
/** Session-fold checkpoint cache paths (~/.dsh/deepseek-quota/checkpoints.json). */
const CHECKPOINTS_DIR = join(homedir(), ".dsh", "deepseek-quota");
const CHECKPOINTS_FILE = join(CHECKPOINTS_DIR, "checkpoints.json");
const CHECKPOINTS_VERSION = 1;
/** Debounce for checkpoint writes; folds can batch many sessions per request. */
const CHECKPOINTS_PERSIST_DEBOUNCE_MS = 3000;
...
function scheduleCheckpointPersist() {
  if (checkpointPersistTimer !== null) return;
  checkpointPersistTimer = setTimeout(() => {
    checkpointPersistTimer = null;
    void persistCheckpoints(foldCheckpoints);
  }, CHECKPOINTS_PERSIST_DEBOUNCE_MS);
}
...
export function apply(ctx, config = {}) {
  ...
  // 会话级折叠检查点在模块级；此处仅启动恢复
  void loadCheckpoints(foldCheckpoints);
...
/** 检查点落盘：{ version, entries: [[id, checkpoint], …] }；任何失败静默。 */
async function persistCheckpoints(map) {
  try {
    const entries = [];
    for (const [id, cp] of map) {
      entries.push([id, {
        revision: cp.revision,
        fromSeq: cp.fromSeq,
        lastProvider: cp.lastProvider ?? null,
        lastModel: cp.lastModel ?? null,
    ...
    await writeFile(tmp, JSON.stringify({ version: CHECKPOINTS_VERSION, entries }), "utf8");
    await rename(tmp, CHECKPOINTS_FILE);
  } catch {
    // cache only: failure degrades to a full fold later
  }
}
...
async function loadCheckpoints(map) {
  try {
    const raw = await readFile(CHECKPOINTS_FILE, "utf8");
    const parsed = JSON.parse(raw);
    if (parsed?.version !== CHECKPOINTS_VERSION || !Array.isArray(parsed.entries)) return;
    for (const [id, cp] of parsed.entries) {
      if (typeof id !== "string" || cp === null || typeof cp !== "object") continue;
      if (typeof cp.revision !== "string" || !Array.isArray(cp.samples)) continue;
      map.set(id, {
        revision: cp.revision,
        fromSeq: typeof cp.fromSeq === "number" && Number.isFinite(cp.fromSeq) ? cp.fromSeq : 0,
        lastProvider: typeof cp.lastProvider === "string" ? cp.lastProvider : undefined,
        lastModel: typeof cp.lastModel === "string" ? cp.lastModel : undefined,
        startedTurn: typeof cp.startedTurn === "number" ? cp.startedTurn : -1,
        samples: cp.samples,
      });
    }
  } catch {
    // first run / corrupted cache：从空开始
  }
}
...
/**
 * 折叠一个会话（带检查点增量）：live 内存路径 → revision 未变零读 →
 * readFrom 增量 → 全量（inspect）。返回 foldSessionUsage 形状 + startedTurn，
 * 或 undefined（会话不存在）。
 */
async function foldSessionCached(ctx, id, { pricing, signal, persistence, revision }) {
  const live = liveEventsOf(ctx, id);
  if (live !== undefined) {
    const folded = foldSessionUsage(live.events, live.seedLength, pricing);
    return { ...folded, startedTurn: startedTurnOfEvents(live.events) };
  }
  if (persistence === undefined) return undefined;
  const cp = foldCheckpoints.get(id);
  if (typeof persistence.readFrom === "function" && typeof revision === "string") {
    if (cp !== undefined && cp.revision === revision) {
  ...
  if (persistence !== undefined && !partial) {
    ...
    try {
      if (typeof persistence.listSnapshots === "function") {
        ...
        snapshots = list.map((s) => ({ id: s.header?.id, revision: s.revision }));
      } else {
        const headers = await persistence.list(signal);
        snapshots = headers.map((h) => ({ id: h.id, revision: void 0 }));
      }
    } catch {
      snapshots = [];
    }
      ...
      const settled = await Promise.all(batch.map(async (snap) => {
        ...
        try {
          const folded = await foldSessionCached(ctx, snap.id, {
            pricing, signal, persistence, revision: snap.revision,
          });
          const rows = [];
          for (const sample of folded?.steps ?? []) {
  ...
  try {
    ...
    if (persistence !== undefined && typeof persistence.listSnapshots === "function") {
      try {
        ...
        snapshotsMap = new Map(snapshots.map((s) => [s.header?.id, s.revision]));
      } catch {
        // revision 未知 → 全量路径
      }
    }
    const folded = await foldSessionCached(ctx, sessionId, {
      pricing, signal: controller.signal, persistence, revision: snapshotsMap.get(sessionId),
    });
    if (folded === undefined) {
      return {
        ok: false,
          ...
          const settled = await Promise.all(batch.map(async (row) => {
            try {
              const child = await foldSessionCached(ctx, row.id, {
                pricing, signal: controller.signal, persistence, revision: snapshotsMap.get(row.id),
              });
