...
async function loadCheckpoints(map) {
  try {
    ...
    for (const [id, cp] of parsed.entries) {
      ...
      map.set(id, {
        revision: cp.revision,
        createdAt: typeof cp.createdAt === "number" ? cp.createdAt : undefined,
        cwd: typeof cp.cwd === "string" ? cp.cwd : undefined,
  ...
  try {
    ...
    }
    {
      ...
      if (hit === void 0 && liveEventsOf(ctx, sessionId) === void 0) {
        // 已被删除：清理残留检查点（免得在盘上长期堆积）
        if (foldCheckpoints.delete(sessionId)) scheduleCheckpointPersist();
      }
    }
