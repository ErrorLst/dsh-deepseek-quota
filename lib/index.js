...
/** 双版本持久化列举归一：优先 listSnapshots（快照形态），否则 list() 后逐项
 * 归一为 {header, revision, sizeBytes}——rc.1 list() 返回裸 SessionHeader，
 * 工作树 handle 缝的 list() 直接返回快照形态。 */
async function normalizePersistenceList(persistence, signal) {
  if (typeof persistence.listSnapshots === "function") {
    return await persistence.listSnapshots(signal);
  }
  // rc.1 缝是位置参数 list(signal)；工作树 handle 缝是 list(options)，须传 { signal }。
  const raw = typeof persistence.open === "function"
...
/** 后台预折叠：按 createdAt 排序取最近创建的会话建立检查点（4 并发、
...
async function warmUpCheckpoints(ctx, pricing) {
  ...
  try {
    const list = await normalizePersistenceList(persistence);
    const withTime = list
      .map((s) => ({ id: s.header?.id, revision: s.revision, identity: identityOf(s.header), at: Number(s.header?.createdAt) || Number(s.header?.updatedAt) || 0 }))
...
/** 工作树 handle 缝的 NotFound 拒绝识别（dsh errors.ts 契约：name 固定）。 */
function isPersistenceNotFound(error) {
 ...
 * readFrom 增量 → 全量（inspect / open+read）。返回 foldSessionUsage
 * 形状 + startedTurn，或 undefined（会话不存在）。identity 来自列举
 * （listSnapshots / 快照形态 list）或全量读取（inspect / handle.header）
 * 的 {createdAt, cwd}；与检查点归属不一致时丢弃检查点回退全量。
 */
async function foldSessionCached(ctx, id, { pricing, signal, persistence, revision, identity }) {
  ...
  // 零读快速路径不依赖 readFrom：宿主契约明确 "equal revisions may be treated
  // as an unchanged log"，工作树宿主无 readFrom 时 revision 一致也应零读命中，
  // 否则检查点缓存只写不读、每次刷新都重读全量并重写 sqlite 行。
  if (sameLog && typeof revision === "string" && cp.revision === revision) {
  ...
  // 全量（首次 / 无 readFrom 增量路径）：rc.1 缝用 inspect；工作树 handle 缝
  // （无 inspect）用 open("read") + handle.read 读全量事件，读毕 close；两者
  // 皆无 → 冷会话视为不可用（返回 undefined，不抛异常）。
  signal?.throwIfAborted();
  let inspection;
  if (typeof persistence.inspect === "function") {
    inspection = await persistence.inspect(id, signal);
  } else if (typeof persistence.open === "function") {
    // 会话不存在 → undefined：与 rc.1 缝 inspect 返回 undefined 的
    // SESSION_NOT_FOUND 语义一致，避免穿透 context 路由报 INTERNAL。优先
    // stat 探测（契约：不存在返回 undefined）；stat→open 竞态中被删除时，
    // open 的 SessionPersistenceNotFoundError 同样归一为不存在。
    let handle;
    try {
      if (typeof persistence.stat === "function") {
        const probe = await persistence.stat(id, { signal });
        if (probe === undefined) return undefined;
      }
      try {
        handle = await persistence.open(id, "read", { signal });
      } catch (error) {
        if (isPersistenceNotFound(error)) return undefined;
        throw error;
      }
      ...
      if (handle !== undefined) {
        try { await handle.close(); } catch { /* 释放失败不阻断折叠 */ }
      }
    }
  } else {
    return undefined;
...
async function collectGlobalSamples(ctx, pricing, signal) {
  ...
  if (persistence !== undefined && !partial) {
    ...
    let listed = false;
    try {
      const list = await normalizePersistenceList(persistence, signal);
    ...
    // 与 loadContext 同一原则：只按权威列举清理。列举失败/中止（spend 超时
    // throwIfAborted、artifact stat 瞬时错误）时空列表不代表会话已删除——
    // 销毁检查点只会把后续折叠打回冷全量（内存 + sqlite 行都不动）。
...
async function loadContext(ctx, sessionId, options) {
  ...
  try {
    const persistence = ctx.get("sessionPersistence");
    // 一次轻量快照拿全部修订号（双版本列举归一）；父会话与每个子代理共用
    // （revision 一致 → 零读）
    let snapshotsMap = new Map();
    let listed = false;
    if (persistence !== undefined) {
      try {
        const snapshots = await normalizePersistenceList(persistence, controller.signal);
        snapshotsMap = new Map(snapshots.map((s) => [s?.header?.id, { revision: s?.revision, identity: identityOf(s?.header) }]));
        listed = true;
      } catch {
        // 列举失败 → revision 未知 → 全量路径
      }
    }
    // 「已被删除」只能按权威列举推断：列举成功且该 id 不在其中、又不在线时，
    // 才清理残留检查点（内存 + sqlite 行）；列举不可用/失败时不得销毁检查点。
    if (listed) {
