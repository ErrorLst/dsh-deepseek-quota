...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        try {
          ...
          balanceStore.snapshot = {
            ...
            spend: {
              ...
              at: now,
              boundaries: map,
              // True when the server fold was interrupted by its timeout
              // budget: several sessions' usage is missing, so window deltas
              // undercount and many rows show 0. Surfaced in the panel.
    ...
    // Long ids/labels (model ids, subagent ids/labels) are cut at this cap
    // so a panel column never bleeds into its neighbor; the full text stays
    // available through the cell's title attribute.
    const MODEL_NAME_MAX = 16;

    /** Hard truncation with an ellipsis; null/undefined renders as "—". */
    function truncateLong(value) {
    ...
    function buildSubagentsPanel(subagents) {
      ...
      const rows = children.map((child, index) =>
        tableRow(
          [
            // Subagent labels (or fallback ids) can be very long: cut at the
            // shared cap; the full label/id stays in the title attribute.
            dataCell(
              React.createElement(
                "span",
                { title: String(child.label ?? child.id ?? "\u2014") },
                truncateLong(child.label ?? child.id),
              ),
              TD_L,
              "id",
            ),
            dataCell(
              React.createElement(
                "span",
                { title: String(child.model ?? "\u2014") },
                truncateLong(child.model),
              ),
              TD_L,
              "model",
            ),
    ...
    function buildBalancePanel(snapshot, primary, tab, onTabChange) {
      ...
      // The global fold can outrun its timeout budget (many persisted
      // sessions): some usage is then missing, window deltas undercount and
      // many rows read 0. Say so instead of silently showing wrong zeros.
      const partialNote = spend !== null && spend.partial === true
        ? React.createElement(
            "div",
            { style: { ...PANEL_NOTE_STYLE, color: "var(--dsw-alias-state-error-primary, #e5484d)" } },
            "\u5168\u5c40\u7edf\u8ba1\u672a\u5b8c\u6574\uff1a\u90e8\u5206\u4f1a\u8bdd\u8d85\u65f6\u672a\u8ba1\u5165\uff0c\u5f53\u524dAPI\u53d8\u5316\u91cf\u504f\u4f4e",
          )
        : null;
      const head = tableRow(
        [
