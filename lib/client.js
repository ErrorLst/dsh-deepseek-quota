...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const balanceStore = {
      snapshot: loadStoredBalance() ?? { state: { status: "loading" }, history: [] },
      listeners: new Set(),
      pollStarted: false,
      subscribe(listener) {
      ...
      async refresh() {
        try {
          ...
          if (data && data.ok) {
            ...
            const history = balanceStore.snapshot.history;
            ...
            balanceStore.snapshot = {
              ...
              history: primary === undefined
                ? history
                : [{ at: Date.now(), total: primary.total, delta }, ...history].slice(0, BALANCE_HISTORY_MAX),
            };
          } else {
    ...
    /**
     * 余额：the last BALANCE_HISTORY_MAX balance refreshes with the delta of
     * each refresh against its predecessor. Fixed at 10 rows — no scrollbar.
     */
    function buildBalancePanel(history, primary) {
      const header = React.createElement(
        "div",
        { style: PANEL_TITLE_STYLE },
        `\u4f59\u989d\u53d8\u52a8 \u00b7 \u6700\u8fd1 ${history.length} \u6b21\u5237\u65b0`,
      );
      ...
      const rows = history.map((entry, index) => {
        ...
        return tableRow(
          [
            dataCell(formatTime(entry.at), TD_L, "time"),
            dataCell(`\u00a5${entry.total}`, TD_R, "total"),
            dataCell(
              color === undefined
                ? formatDelta(entry.delta)
                : React.createElement("span", { style: { color } }, formatDelta(entry.delta)),
              TD_R,
              "delta",
            ),
          ],
      ...
      return React.createElement(
        ...
        React.createElement(
          "div",
          { style: { maxHeight: `${BALANCE_HISTORY_MAX * 22 + 24}px`, overflowY: "hidden" } },
          React.createElement(
            "table",
            { style: TABLE_STYLE },
            React.createElement("thead", null, head),
    ...
    function QuotaContextLine({ sessionId, useSession, useProjection, useSessions }) {
      ...
      if (hover === "balance") panelContent = buildBalancePanel(balanceHistory, balancePrimary);
      ...
      if (panelContent !== null) {
        const panelElement = React.createElement(
          "div",
          { ref: panelRef, style: PANEL_STYLE, role: "tooltip" },
          panelContent,
        );
        panel = ReactDOM !== null
